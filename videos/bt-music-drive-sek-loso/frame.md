...
description: >
  ...
  Kanit in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  # — reading ramp —
  body:    { fontFamily: "Kanit", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead:    { fontFamily: "Kanit", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Kanit", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Kanit, lowercase, negative tracking) —
  h3:      { fontFamily: "Kanit", cqw: 2.8, weight: 600, lineHeight: 1.2, lower: true }
  quote-text:{ fontFamily: "Kanit", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em", lower: true }
  h2:      { fontFamily: "Kanit", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em", lower: true }
  stat-value:{ fontFamily: "Kanit", cqw: 5.5, weight: 900, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Kanit", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em", lower: true }
  fadelist-item:{ fontFamily: "Kanit", cqw: 7.5, weight: 900, lineHeight: 1.0, tracking: "-0.03em", lower: true }
  quote-mark:{ fontFamily: "Kanit", cqw: 10.0, weight: 900, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Kanit", cqw: 10.5, weight: 900, lineHeight: 0.9, tracking: "-0.04em", lower: true }
  display: { fontFamily: "Kanit", cqw: 13.0, weight: 900, lineHeight: 0.88, tracking: "-0.04em", lower: true }
...
- **Fonts** — already set to **Kanit** (display) / **Inter** (body); ignore any preset font name lingering in prose.
...
text and becomes graphic primitive.** Kanit `display` at 13cqw puts a single lowercase word
...
**Kanit** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Kanit 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Kanit body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Kanit `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Kanit display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Kanit `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Kanit lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Kanit `display`/`h1`
...
row of three top-border-only stat-cards — big Kanit-900 numeral in `{colors.fire-orange}`, Kanit
...
**Ground** ink-black. **Composes** fadelist (3 stacked Kanit-900 words at 1.0/0.5/0.22), fadelist-title.
...
Kanit `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Kanit display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Kanit display; never add a second accent color.
...
- No serif companion; chrome is never Kanit.
...
- **Type** — Kanit lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Kanit + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
