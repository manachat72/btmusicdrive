...
- **Product Studio** (2026-08-18): `npm run mkt:studio` → http://localhost:4777 — ลงสินค้าใหม่ / แก้ไขสินค้าเดิม / QR ครบในที่เดียว
  ...
  - **คลังรูปกลางชุดใหม่ `mid/<slug>/` บน R2** (2026-09-20): `products/<code>/` ชุดเก่าตรงกับรูปบนเว็บแค่ 14/54 (code ซ้ำ/เพี้ยนหลังเปลี่ยนเลขโฟลเดอร์ NAS) — ชุดใหม่ตั้งชื่อด้วย slug เลยชนกันไม่ได้ และไม่ทับของเดิม · สร้างด้วย `node scripts/rebuild-mid-images.js [--apply] [--slug <slug>]` (ลำดับรูป = ลำดับใน DB = ที่เห็นบนเว็บ · แต่ละใบหาต้นฉบับความละเอียดสูงในโฟลเดอร์ NAS ด้วย dHash ก่อน ไม่เจอค่อยใช้ไฟล์เว็บ) → ลิงก์เก็บใน `reports/mid-images.json` · เติมลงเทมเพลตแพลตฟอร์มด้วย `node scripts/fill-xlsx-images.js "<ไฟล์.xlsx>" --apply` (จับคู่ด้วย SKU, เขียนไฟล์ใหม่ลงท้าย `-images-fixed.xlsx`) · **Shopee รับแค่ jpg/png ≥500×500 ≤2MB ห้ามใส่ลิงก์ webp ของหน้าเว็บตรง ๆ**
  - ทำงานได้แม้ NAS ไม่ได้ต่อ (อัปโหลดรูปผ่านหน้าเว็บ → ต้นฉบับขึ้น R2 อย่างเดียว)
  - ⚠ **เลขโฟลเดอร์ NAS ≠ code marketplace แล้ว** (2026-09-12): โฟลเดอร์ใน `Z:\photos\Product` ถูกเปลี่ยนเลขใหม่หมด (เช่น 3 ช่า `21-`→`45-`) แต่ `catalog.json` + R2 `products/<code>/` (ลิงก์ xlsx) ยังเป็นเลขเดิม — **ห้ามเอาเลขโฟลเดอร์ใหม่ไปใช้เป็น code** จะทับรูป marketplace ของสินค้าตัวอื่น · อัปเดตเฉพาะรูปเว็บได้ (`buildWebImages` → `planProductWebImages` + `r2.putMany` → DB) · จับคู่สินค้า↔โฟลเดอร์ใช้ dHash ของรูปที่ไม่ซ้ำโฟลเดอร์อื่น + ชื่อโฟลเดอร์ (เดาจากเลข/SKU ไม่ได้)
    - ปุ่ม "🖼 ใช้ 9 รูปแรกขึ้นเว็บ" (`/api/replace-images-from-nas`, 2026-09-13): **ไม่เลือก code = ทำเฉพาะรูปเว็บ** ไม่แตะ marketplace · ถ้าเลือก code ที่ catalog ไม่มี slug จะเทียบชื่อโฟลเดอร์ (ตัดเลขนำหน้า) กับ `dirName` ผ่าน `assertCodeMatchesFolder` — ไม่ตรงจะบล็อกและบอก code ที่ถูก (เช่น คำภีร์ = โฟลเดอร์ 12 แต่ code 14)
