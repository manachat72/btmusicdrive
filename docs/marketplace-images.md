...
## 🎛 Listing Studio — ลงสินค้าผ่านหน้าเว็บ (local)

```bash
npm run mkt:studio     # เปิด http://localhost:4777
```

**➕ ลงสินค้าใหม่ (ลงเว็บก่อน แล้วค่อยกระจาย):**
กดปุ่มมุมขวาบน → ตั้งชื่อสั้น (เช่น "ฮิตยุค90") + ราคา/ความจุ → เลือกโฟลเดอร์รูปบน NAS
หรืออัปโหลดรูปให้สร้างโฟลเดอร์ให้ (`<code>-<ชื่อ>\หลัก_01.jpg …`) → แนบ .txt รายชื่อเพลง →
ตกลง → ระบบแปลงรูปขึ้น R2 + ทำ SEO ตั้งชื่อ/รายละเอียด/หมวด/Tags → **รีวิวแก้ได้** →
login แอดมิน → ยืนยันลงเว็บ (POST /api/products จริง) → sync products.json + อัปเดต listings อัตโนมัติ
จากนั้นเลือกสินค้าใหม่ในรายการเพื่อกดกระจายแพลตฟอร์มต่อ
(หลังลงเว็บควรรัน `npm run build` + commit เพื่ออัปเดต fallback products.json บนเว็บ — ใช้ /check-commit-push)

**กระจายไปแพลตฟอร์ม:** เลือกสินค้าจากรายการ (มีรูป+ค้นหาได้) → เห็นรูปทั้ง 9 → กดปุ่มแพลตฟอร์ม → ได้ไฟล์ .xlsx ทันที:
